...
const availableTitles = availableBooks.map((availableBook) => availableBook.title);
...
const totalCheckouts = books.reduce((sum, book) => {
  return sum + book.checkouts;
}, 0);
...
const mostCheckouts = books.reduce((sum, book) => {
  if(book.checkouts > sum){
